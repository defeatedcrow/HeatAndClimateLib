...
public class ClimateEffectiveCategory implements IRecipeCategory {
	...
	public void setRecipe(IRecipeLayout recipeLayout, IRecipeWrapper recipeWrapper, IIngredients ingredients) {
		...
		recipeLayout.getIngredientsGroup(ClimateTypes.TEMP).init(0, true, new HeatTierRenderer(), 73, 20, 40, 5, 0, 0);
		recipeLayout.getIngredientsGroup(ClimateTypes.TEMP).set(0, temps.get(0));

		List<DCHumidity> hums = wrapper.getHums();
		recipeLayout.getIngredientsGroup(ClimateTypes.HUM).init(0, true, new HumidityRenderer(), 73, 34, 40, 5, 0, 0);
		recipeLayout.getIngredientsGroup(ClimateTypes.HUM).set(0, hums.get(0));

		List<DCAirflow> airs = wrapper.getAirs();
		recipeLayout.getIngredientsGroup(ClimateTypes.AIR).init(0, true, new AirflowRenderer(), 73, 48, 40, 5, 0, 0);
